...
def get_column_types(view: str) -> Dict[str, str]:
    """Fetch column names and their Snowflake data types."""
    sql = f"DESC TABLE {view}"
    df = run_query(sql)
    if df is None or df.empty:
        return {}
    return {row[0].upper(): row[1].upper() for row in df.itertuples(index=False)}
# ================================================================
SNOWFLAKE_TIME_FMT = "YYYY-MM-DD HH24:MI:SS.FF3"
def cast_ts(col: str, view: str) -> str:
    col_types = get_column_types(view)
        ...
        if ftype == "date_range" and isinstance(val, dict):
            ...
            if start and end and col:
                ...
                if time_col and col.upper() == time_col.upper():
                    ts_expr = cast_ts(col, dataset_cfg.get("snowflake_view", ""))
...
def fetch_dataset_df(dataset_cfg: Dict[str, Any], filters_cfg: List[Dict[str, Any]], filter_values: Dict[str, Any]) -> pd.DataFrame:
    ...
    if time_col:
        ...
        select_cols.append(f"{cast_ts(time_col, view)} AS LOAD_TS") # parsed
